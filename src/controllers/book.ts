...
export const getBook = controllerFactory(async (req: Request, res: Response) => {
  const book = await Book.findById(req.params.id);
  if (!book) throw new NotFoundError("Book not found");
  res.status(200).json(book);
});

export const createBook = controllerFactory(async (req: Request, res: Response) => {
...
export const deleteBook = controllerFactory(async (req: Request, res: Response) => {
  const book = await Book.findById(req.params.id);
  if (!book) throw new NotFoundError("Book not found");
...
export const listBooks = controllerFactory(async (req: Request, res: Response) => {
  const schema = z.object({
